...
<template>
  <header class="h-[3.75rem] px-3 md:px-6 py-3 grid grid-cols-2 lg:grid-cols-3 rounded-2xl border border-zinc-100 bg-white text-zinc-900">
    <div class="flex">
    ...
    <nav v-if="navigation" class="hidden lg:flex justify-center">
      <ol class="text-[1.125rem] flex gap-4 leading-5">
        <li v-for="item in navigation" :key="item._path">
          <UButton size="md" variant="ghost" color="gray" :to="item._path" :ui="uiButton" active-class="bg-primary bg-opacity-30">
            {{ item.title }}
          </UButton>
        </li>
      </ol>
    ...
    <div class="flex justify-end items-center gap-3">
      <UTooltip text="Open Search" :shortcuts="[metaSymbol, 'K']">
        <UButton size="md" icon="i-heroicons-magnifying-glass-20-solid" color="gray" variant="ghost" :ui="uiButton" @click="openSearch = true">
          Search
        </UButton>
      </UTooltip>
