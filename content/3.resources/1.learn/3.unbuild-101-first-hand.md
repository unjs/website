...
<!-- infer from package.json because your must fill it since you will publish a package to npm -->

<!-- peut être utilisé avec du js et du jsdoc pour le typing. pour autant, ilest plutôt dimensionné polru être utilisé avec typescript  -->
...
<!-- explain that unbuild is used to publish package on npm -->
...
By default, [unjs/unbuild](https://github.com/unjs/unbuild) works out of the box without configuration by inferring the configuration founded in the `package.json` used by `npm` to publish our package.
