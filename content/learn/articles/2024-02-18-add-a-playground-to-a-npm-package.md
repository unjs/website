---
title: Add a Playground to a npm Package
...
[In the previous article](https://unjs.io/learn/articles/2024-02-17-create-a-npm-package) of this series we setup a npm package from scratch. We added a `package.json`, a simple JavaScript file and unbuild to build our package before publishing it.

> [!NOTE]
> For a better understanding of this article, we recommend to read [the previous one](https://unjs.io/learn/articles/2024-02-17-create-a-npm-package).

Now, and before learning how to publish it, we'll see how to **try our package locally** with a playground.

## A Playground

The main question that we should have in mind is: "What is a playground and why should I add one to my package?"

A playground is a simple way to try your package locally within the same repository. This avoid us to made complex manipulation to verify that your package is working as expected and help others to try the package. For open source or with a team, it's important to have a **simple way** to try the package.

> [!TIP]
> The idea is to try it in real condition. Manual test does not replace unit tests.
...
It's important to note that the playground is not part of our package since it's not added in the `files` field of our `package.json`. This means that the playground will not be published on npm when we publish our package.
...
There is many ways to add a playground to our package. We will see 3 of them and choosing one or the other depends on our needs and our preferences. Each of them have their own advantages and disadvantages.

> [!IMPORTANT]
> This is not mandatory to manage our playground using a package manager. We can simply import from `src` with `../src`. It could be enough for a lot of cases. But in case we need a more complex playground, let's see how to do it.

We will see how to add a playground to our package using bun and pnpm. Npm does not support the `workspace` protocol, useful to link our package to the playground. If we use npm, simply use a direct import from `../src`.
...
This is the simplest solution if your package run in Node.js like a set of utility functions. In the `playground` directory, we can import our package directly from the `src` directory.
...
Instead, use a minimal [Vite](https://vitejs.dev/) project inside our playground to easily support TypeScript without building the package. This can be achieve using the [direct reference](#direct-reference) or a [pnpm workspace](#monorepo-with-pnpm).

> [!TIP]
> Please read [direct reference](#direct-reference) or [pnpm workspace](#monorepo-with-pnpm) section for a better playground setup. This section is for educational purpose only.

### Monorepo with pnpm

Pnpm have a concept of workspace that allow us to manage multiple packages in the same repository. This means that our package will be a workspace and the playground will be another workspace that will consume our package as a dependency, as if it was published on npm.
...
There is two ways to do this: a _passive_ watcher and an active watcher.

The active watcher is the most common patter. You start a watcher that watch for changes and rebuild the package when a change is detected.
...
Unbuild take the second approche with the `--stub` option. Run 'unbuild --stub' to generate the passive watcher and take a look at the `dist` directory. You will see a `index.mjs` file that load on the fly the `src` directory.
...
In the `index.mjs` file in the `playground` directory, setup with a monorepo with pnpm, we can use our package:
...
Update the `src/index.ts` file and re-run the `playground/index.mjs` file. It's working! The playground take the changes but the package is not rebuild. No need to run `pnpm build` every time we make a change or to wait for a watcher to rebuild the package.
...
With [`npm`](https://docs.npmjs.com/cli/v10/commands/npm-link), [`pnpm`](https://pnpm.io/cli/link) or [`bun`](https://bun.sh/docs/cli/link), we ca use the `link` command.
...
During this article, we learned why a playground is important as a developer to try out package locally. We them saw different ways to add a playground to our package and how to use it.

This was an occasion to discover the `unbuild --stub` feature and how to use it within a monorepo with pnpm.

In future articles, we will see how to how to **publish the package** on npm, how to **create a package with TypeScript** and how to **automate the process** with [unjs/changelogen](https://changelogen.unjs.io/).
